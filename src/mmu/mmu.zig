...
pub const Mmu = struct {
    ...
    pub fn init(cartridge: *Cartridge, wram: *memory.WRam, hram: *memory.HRam, io: *Io, ppu: *Ppu) Mmu {
        return .{
            .cartridge = cartridge,
            .wram = wram,
            .hram = hram,
    ...
    pub fn readByte(self: *const Mmu, address: u16) u8 {
        return switch (address) {
            ...
            0x8000...0x9FFF => self.ppu.readByte(address), // VRAM (8 KB) – video memory
            0xA000...0xBFFF => self.cartridge.readByte(address), // External RAM in cartridge
            ...
            0xFE00...0xFE9F => self.ppu.readByte(address), // OAM (sprite attribute memory, 160 bytes)
            0xFEA0...0xFEFF => 0, // Unusable / forbidden memory
            0xFF00...0xFF7F => self.io.readByte(address), // I/O registers
            0xFF80...0xFFFE => self.hram.readByte(address - 0xFF80), // High Ram (HRAM)
            0xFFFF => self.io.readByte(address), // Interrupt Enable register
        };
    }
    ...
    pub fn writeByte(self: *Mmu, address: u16, value: u8) void {
        switch (address) {
            ...
            0x8000...0x9FFF => self.ppu.writeByte(address, value), // VRAM – video memory
            ...
            0xFE00...0xFE9F => self.ppu.writeByte(address, value), // OAM (sprite attribute memory, 160 bytes)
            0xFEA0...0xFEFF => {}, // Unusable / forbidden memory – writes ignored
            0xFF00...0xFF7F => self.io.writeByte(address, value), // I/O registers
